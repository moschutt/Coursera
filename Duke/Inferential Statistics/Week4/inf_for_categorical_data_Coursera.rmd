...
n = nrow(us12)
prop = atheist_prop * (1 - atheist_prop)
...
inference(y = response, data = de12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")

# type your code for the Exercise here, and Knit
es12 <- atheism %>%
  filter(nationality == "Spain" , atheism$year == "2012")
inference(y = response, data = es12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
```
...
es_atheism <- atheism %>%
  ...
  group_by(year) %>%
  summarize(n=n(), p=mean(response == 'atheist'))

p_2005 <- es_atheism %>% filter(year==2005) %>% .$p
n_2005 <- es_atheism %>% filter(year==2005) %>% .$n
p_2012 <- es_atheism %>% filter(year==2012) %>% .$p
n_2012 <- es_atheism %>% filter(year==2012) %>% .$n

p_pool <- atheism %>%
  filter(nationality == 'Spain') %>% 
  summarize(p = mean(response == 'atheist')) %>% .$p
p <- p_pool * (1 - p_pool)

SE <- sqrt(p/n_2005 + p/n_2012)
T <- p_2005 - p_2012
Z <- T / SE
p_value <- pnorm(Z, lower.tail=(Z < 0)) * 2
...
p_2005 <- us_atheism %>% filter(year==2005) %>% .$p
n_2005 <- us_atheism %>% filter(year==2005) %>% .$n
p_2012 <- us_atheism %>% filter(year==2012) %>% .$p
n_2012 <- us_atheism %>% filter(year==2012) %>% .$n
...
# ME = z* * sqrt(p * (1 - p) / n)  Where p = 0.5 because we don't know it. and ME = 0.01 (1%)
(1.96 * .5 / 0.01)^2
```
